...
***  New implementation with email templates  ************************
*    DATA(lo_bcs) = cl_bcs=>create_persistent( ).
*    CASE iv_rel_status.
*      WHEN '01'.
*        DATA(lo_email_api) = cl_smtg_email_api=>get_instance( iv_template_id = 'ZMJE_EML_TPL_SUBMIT' ).
*        DATA(ls_eml_param) = VALUE z_c_mje_eml_tpl(
*            initiator_id    = iv_initiator
**        approver_id     = iv_approvers
*            doc_key         = iv_bt_id
*            email_link      = iv_email_link ).
*      WHEN OTHERS.
*        lo_email_api = cl_smtg_email_api=>get_instance( iv_template_id = 'ZMJE_EML_TPL_RELEASE' ).
*        ls_eml_param = VALUE z_c_mje_eml_tpl(
*            approver_id   = iv_initiator
*            status        = COND char10(    WHEN iv_rel_status = '02'   THEN 'Released'
*                                            WHEN iv_rel_status = '03'   THEN 'rejected'
*                                            ELSE '' )
*            doc_key       = iv_bt_id
*            email_link    = iv_email_link ).
...
*    DATA(lo_rec_initiator) = cl_cam_address_bcs=>create_user_home_address( CONV #( iv_initiator ) ).
*      lo_bcs->add_recipient( EXPORTING i_recipient = lo_rec_initiator ).
*    LOOP AT lt_recipients ASSIGNING FIELD-SYMBOL(<str_recipient>).
*      DATA(lo_recipient) = cl_cam_address_bcs=>create_user_home_address( CONV #( <str_recipient>-receiver ) ).
